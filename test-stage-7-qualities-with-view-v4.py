...
@dataclass
class LaserLine:
    """UE-like struct: X, Y = center; Length; Rotation (degrees 0..360)."""
    x: float
    y: float
    length: float
    rotation_deg: float
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(description="Send /laser/line (N*4 floats) + live viz.")
...
def build_lines(t: float, cfg: Config, spin_phase: List[float], orbit_phase: List[float]) -> List[LaserLine]:
    """Compute line structs for the current time, with synchronized radial pulse + length modulation."""
    ...
    for i in range(cfg.lines):
        ...
        out.append(LaserLine(x=x, y=y, length=len_now, rotation_deg=rot_deg))
    return out
...
def flatten_payload(lines: List[LaserLine]) -> List[float]:
    payload: List[float] = []
    for ln in lines:
        payload.extend([float(ln.x), float(ln.y), float(ln.length), float(ln.rotation_deg)])
    return payload
...
def setup_plot(cfg: Config) -> Tuple[plt.Figure, plt.Axes, List[Line2D], Optional[Line2D], plt.Text, plt.Text]:
    ...
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_title("Laser Lines: orbit=5s, spin=2s, pulse=radial in/out, length=sin(60..160)")

    ax.grid(True, alpha=0.2)

    header_text = fig.text(
        0.01, 0.97,
        f"OSC → {cfg.ip}:{cfg.port}  addr=/laser/line  values={cfg.lines*4}  "
