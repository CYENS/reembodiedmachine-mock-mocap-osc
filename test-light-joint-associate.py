...
def replay(
    ...
    try:
        while True:
            ...
            n_joints = 23
            n_lights = 16
            associated_joints = np.random.randint(-1, n_joints, size=n_lights)
            client.send_message("/light/joint/associate", associated_joints.tolist())
            dt = time.time() - t0
            sleep = period - dt
            if sleep > 0:
                time.sleep(sleep)
...
def main() -> None:
    ...
    print(
        f"▶️  Sending /light/joint/associate "
